...
class _FamilyRequestsScreenState extends State<FamilyRequestsScreen> {
  ...
  Future<void> _acceptdocumentRequest(
      String sender, String userId, String docname, String docid) async {
    final QuerySnapshot<Map<String, dynamic>> querySnapshot =
        await FirebaseFirestore.instance
            .collection('users')
            .doc(sender)
            .collection('docAccess')
            .where('docowner', isEqualTo: widget.user.username.toString())
            .where('docname', isEqualTo: docname)
            .limit(1)
            .get();
    String senderdocumentid = querySnapshot.docs.first.id;
    await _firestore
        .collection('users')
        .doc(sender.toString())
        .collection('docAccess')
        .doc(senderdocumentid.toString())
        .update({'status': 'accepted'});
    await _firestore
        .collection('users')
